# 🚗 STAR Security Contract for GeographicBoundary API
This README provides an overview of the security architecture and implementation requirements for the **Automotive Retail Systems GeographicBoundary API**.
All developers and integrators must adhere to these standards to ensure the integrity and confidentiality of automotive retail GeographicBoundary data.
...
Most endpoints (e.g., `/geographic-boundaries`, `/geographic-boundaries/batch`) require authentication. A request is considered authorized if it satisfies **either** the OAuth2 requirements **or** provides a valid Member API Key:
...
* `GET /geographic-boundaries/health`: Used by load balancers and uptime monitors.
...
When using the `/geographic-boundaries/exports` endpoint, the system sends a POST notification to your `webhook_url`. To secure this communication:
